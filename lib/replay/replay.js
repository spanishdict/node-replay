...
Replay = (function(superClass) {
  extend(Replay, superClass);
  ...
  Replay.prototype.passThrough = function() {
    var host, hosts, i, len, results;
    hosts = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    this.reset.apply(this, hosts);
    results = [];
    for (i = 0, len = hosts.length; i < len; i++) {
      host = hosts[i];
      results.push(this._passThrough[host] = true);
    }
    return results;
  };
  ...
  Replay.prototype.drop = function() {
    ...
    for (i = 0, len = hosts.length; i < len; i++) {
      host = hosts[i];
      results.push(this._dropped[host] = true);
    }
    return results;
  };
  ...
  Replay.prototype.localhost = function() {
    ...
    for (i = 0, len = hosts.length; i < len; i++) {
      host = hosts[i];
      results.push(this._localhosts[host] = true);
    }
    return results;
  };
  ...
  Replay.prototype.reset = function() {
    ...
    for (i = 0, len = hosts.length; i < len; i++) {
      ...
      results.push(delete this._dropped[host]);
    }
    return results;
  };
